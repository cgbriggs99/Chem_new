...
#include "../Project-12/tdhf_rpa1.hpp"
#include "../Project-12/tdhf_rpa2.hpp"
...
private:
	compchem::TDHFEnergyStrategy *strat1, *strat2;
...
public:
	CISTest(const char *dir) {
		strat1 = new compchem::strategies::RPATDHFEnergyStrategy<
		        compchem::strategies::LapackEigenvalues<double> >();
		strat2 =
		        new compchem::strategies::EERPATDHFEnergyStrategy<
		                compchem::strategies::LapackEigenvalues<double>,
	...
	void runTest() {
		...
		compchem::Matrix<double> *rpa1_energy =
		        (compchem::Matrix<double> *) &strat1->TDHFEnergies(*sofock,
		                *sotei, mol->nelectron()), *rpa2_energy =
		        (compchem::Matrix<double> *) &strat2->TDHFEnergies(*sofock,
		                *sotei, mol->nelectron());

		std::vector<double> vec1, vec2;
		for(int i = 0; i < rpa1_energy->getShape(0); i++) {
			vec1.push_back(rpa1_energy->getEntry(i));
		}
		for(int i = 0; i < rpa2_energy->getShape(0); i++) {
			vec2.push_back(rpa2_energy->getEntry(i));
		}

		compareList(vec1, "tdhf_rpa1");
		compareList(vec2, "tdhf_rpa2");
