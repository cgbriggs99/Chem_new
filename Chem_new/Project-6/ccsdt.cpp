...
#ifndef __CCSDT_CPP__
#define __CCSDT_CPP__
...
static compchem::Matrix<double> &calculateSOTEI(
	...
	compchem::Matrix<double> *out = new compchem::Matrix<double>(
	    { 2 * tei.getShape(0), 2 * tei.getShape(0), 2 * tei.getShape(0), 2
	          * tei.getShape(0) }), *motei = new compchem::Matrix<double>(
	    { tei.getShape(0), tei.getShape(0), tei.getShape(0), tei.getShape(0) });
				...
				for(int s = 0; s < 2 * tei.getShape(0); s++) {
					out->setEntry(
					    motei->getEntry(p / 2, r / 2, q / 2, s / 2)
					        * ((p % 2 == r % 2)? 1: 0)
					        * ((q % 2 == s % 2)? 1: 0)
					        - motei->getEntry(p / 2, s / 2, q / 2, r / 2)
					            * ((p % 2 == s % 2)? 1: 0)
					            * ((q % 2 == r % 2)? 1: 0), p, q, r, s);
				}
			}
...
static compchem::Matrix<double> &calculateSOFock(
	...
	for(int i = 0; i < 2 * fock.getShape(0); i++) {
		for(int j = 0; j < 2 * fock.getShape(0); j++) {
			out->setEntry(
			    mofock->getEntry(i / 2, j / 2) * ((i % 2 == j % 2)? 1: 0), i,
			    j);
		}
	}
	delete mofock;
					...
					for(int f = occupied; f < sofock.getShape(0); f++) {
						sum3 += (t2.getEntry(m, n, a, f)
						    + (t1.getEntry(m, a) * t1.getEntry(n, f)
						        - t1.getEntry(m, f) * t1.getEntry(n, a)) / 2)
						    * sotei.getEntry(m, n, e, f) / 2;
					}
				}
			}
					...
					for(int f = occupied; f < sofock.getShape(0); f++) {
						sum3 += (t2.getEntry(i, n, e, f)
						    + (t1.getEntry(i, e) * t1.getEntry(n, f)
						        - t1.getEntry(i, f) * t1.getEntry(n, e)) / 2)
						    * sotei.getEntry(m, n, e, f) / 2;
					}
				}
			}
		...
		for(int n = 0; n < occupied; n++) {
			for(int i = 0; i < occupied; i++) {
				for(int j = 0; j < occupied; j++) {
					...
					out.setEntry(
					    sotei.getEntry(m, n, i, j) + sum1 - sum2 + sum3, m, n,
					    i, j);
				}
			}
		}
		...
		for(int b = occupied; b < sofock.getShape(0); b++) {
			for(int e = occupied; e < sofock.getShape(0); e++) {
				for(int f = occupied; f < sofock.getShape(0); f++) {
					...
					out.setEntry(
					    sotei.getEntry(a, b, e, f) - sum1 + sum2 + sum3, a, b,
					    e, f);
				}
			}
		}
		...
		for(int b = occupied; b < sofock.getShape(0); b++) {
			for(int e = occupied; e < sofock.getShape(0); e++) {
				for(int j = 0; j < occupied; j++) {
					...
					out.setEntry(
					    sotei.getEntry(m, b, e, j) + sum1 - sum2 - sum3, m, b,
					    e, j);
				}
			}
		}
...
static void calculateT1(const compchem::AbstractMatrix<double> &sofock,
	...
	for(int i = 0; i < occupied; i++) {
		for(int a = occupied; a < sofock.getShape(0); a++) {
			...
			if(fabs(
			    (sofock.getEntry(i, a) + sum1 - sum2 + sum3 - sum4 - sum5 - sum6)
			        / (sofock.getEntry(i, i) - sofock.getEntry(a, a))) >= 10) {
				throw(new std::exception());
			...
			out.setEntry(
			    (sofock.getEntry(i, a) + sum1 - sum2 + sum3 - sum4 - sum5 - sum6)
			        / (sofock.getEntry(i, i) - sofock.getEntry(a, a)), i, a);
		}
	}
}
			...
			for(int a = occupied; a < sofock.getShape(0); a++) {
				for(int b = occupied; b < sofock.getShape(0); b++) {
					...
					if(sofock.getEntry(i, i) + sofock.getEntry(j, j)
					    - sofock.getEntry(a, a) - sofock.getEntry(b, b) == 0) {
						throw(new std::exception());
					}

					if(fabs(
					    (sotei.getEntry(i, j, a, b) + sum1 - sum2 - sum3 + sum4
					        + sum5 + sum6 + sum7 - sum8 - sum9 + sum10 + sum11
					        - sum12 + sum13 - sum14)
					        / (sofock.getEntry(i, i) + sofock.getEntry(j, j)
...
static void calculateT3C(const compchem::AbstractMatrix<double> &sofock,
                         const compchem::AbstractMatrix<double> &sotei,
                         const compchem::AbstractMatrix<double> &t1,
                         const compchem::AbstractMatrix<double> &t2,
                         int occupied, compchem::AbstractMatrix<double> &out) {
	for(int i = 0; i < occupied; i++) {
		for(int j = 0; j < occupied; j++) {
			for(int k = 0; k < occupied; k++) {
				for(int a = occupied; a < out.getShape(0); a++) {
					for(int b = occupied; b < out.getShape(0); b++) {
						for(int c = occupied; c < out.getShape(0); c++) {
							double sum1 = 0, sum2 = 0, sum3 = 0, sum4 = 0,
							        sum5 = 0, sum6 = 0, sum7 = 0, sum8 = 0,
							        sum9 = 0;
							for(int e = occupied; e < out.getShape(0); e++) {
								sum1 += t2.getEntry(j, k, a, e)
								    * sotei.getEntry(e, i, b, c);
								sum2 += t2.getEntry(i, k, a, e)
								    * sotei.getEntry(e, j, b, c);
								sum3 += t2.getEntry(j, i, a, e)
								    * sotei.getEntry(e, k, b, c);

								sum4 += t2.getEntry(j, k, b, e)
								    * sotei.getEntry(e, i, a, c);
								sum5 += t2.getEntry(i, k, b, e)
								    * sotei.getEntry(e, j, a, c);
								sum6 += t2.getEntry(j, k, b, e)
								    * sotei.getEntry(e, k, a, c);

								sum7 += t2.getEntry(j, k, c, e)
								    * sotei.getEntry(e, i, b, a);
								sum8 += t2.getEntry(i, k, c, e)
								    * sotei.getEntry(e, j, b, a);
								sum9 += t2.getEntry(j, i, c, e)
								    * sotei.getEntry(e, k, b, a);
							}

							for(int m = 0; m < occupied; m++) {
								sum1 += t2.getEntry(i, m, b, c)
								    * sotei.getEntry(m, a, j, k);
								sum2 += t2.getEntry(j, m, b, c)
								    * sotei.getEntry(m, a, i, k);
								sum3 += t2.getEntry(k, m, b, c)
								    * sotei.getEntry(m, a, j, i);

								sum4 += t2.getEntry(i, m, a, c)
								    * sotei.getEntry(m, b, j, k);
								sum5 += t2.getEntry(j, m, a, c)
								    * sotei.getEntry(m, b, i, k);
								sum6 += t2.getEntry(k, m, a, c)
								    * sotei.getEntry(m, b, j, i);

								sum7 += t2.getEntry(i, m, b, a)
								    * sotei.getEntry(m, c, j, k);
								sum8 += t2.getEntry(j, m, b, a)
								    * sotei.getEntry(m, c, i, k);
								sum9 += t2.getEntry(k, m, b, a)
								    * sotei.getEntry(m, c, j, i);
							}
							out.setEntry(
...
static void calculateT3D(const compchem::AbstractMatrix<double> &sofock,
                         const compchem::AbstractMatrix<double> &sotei,
                         const compchem::AbstractMatrix<double> &t1,
                         const compchem::AbstractMatrix<double> &t2,
                         int occupied, compchem::AbstractMatrix<double> &out) {
			...
			for(int k = 0; k < occupied; k++) {
				for(int a = occupied; a < out.getShape(0); a++) {
					for(int b = occupied; b < out.getShape(0); b++) {
						for(int c = occupied; c < out.getShape(0); c++) {
							out.setEntry(
							    (t1.getEntry(i, a) * sotei.getEntry(j, k, b, c)
							        - t1.getEntry(j, a)
							            * sotei.getEntry(i, k, b, c)
							        - t1.getEntry(k, a)
							            * sotei.getEntry(j, i, b, c)
							        - t1.getEntry(i, b)
							            * sotei.getEntry(j, k, a, c)
							        + t1.getEntry(j, b)
							            * sotei.getEntry(i, k, a, c)
							        + t1.getEntry(k, b)
							            * sotei.getEntry(j, i, a, c)
							        - t1.getEntry(i, c)
							            * sotei.getEntry(j, k, b, a)
							        + t1.getEntry(j, c)
							            * sotei.getEntry(i, k, b, a)
							        + t1.getEntry(k, c)
							            * sotei.getEntry(j, i, b, a))
							        / (sofock.getEntry(i, i)
							            + sofock.getEntry(j, j)
							            + sofock.getEntry(k, k)
							            - sofock.getEntry(a, a)
...
double compchem::strategies::DefaultCCSDTCorrection::CCEnergy(
	    ...
	    { 2 * fock.getShape(0), 2 * fock.getShape(0) }), *w =
	        new compchem::Matrix<double>(
	            { 2 * fock.getShape(0), 2 * fock.getShape(0), 2
	                  * fock.getShape(0),
	              2 * fock.getShape(0) }), *t1 = new compchem::Matrix<double>(
	    { 2 * fock.getShape(0), 2 * fock.getShape(0) }), *t2 =
	        new compchem::Matrix<double>(
	            { 2 * fock.getShape(0), 2 * fock.getShape(0), 2
	                  * fock.getShape(0),
	              2 * fock.getShape(0) }), *hold1 =
	        new compchem::Matrix<double>(
	            { 2 * fock.getShape(0), 2 * fock.getShape(0) }), *hold2 =
	        new compchem::Matrix<double>(
	            { 2 * fock.getShape(0), 2 * fock.getShape(0), 2
	                  * fock.getShape(0),
	              2 * fock.getShape(0) }), *t3d = new compchem::Matrix<double>(
	    { 2 * fock.getShape(0), 2 * fock.getShape(0), 2 * fock.getShape(0), 2
	          * fock.getShape(0) }), *t3c = new compchem::Matrix<double>(
	    { 2 * fock.getShape(0), 2 * fock.getShape(0), 2 * fock.getShape(0), 2
	          * fock.getShape(0) });
	...
	for(int i = 0; i < nelectrons; i++) {
		for(int j = 0; j < nelectrons; j++) {
			for(int k = 0; k < nelectrons; k++) {
				for(int a = nelectrons; a < t1->getShape(0); a++) {
					for(int b = nelectrons; b < t1->getShape(0); b++) {
						for(int c = nelectrons; c < t1->getShape(0); c++) {
							sum += t3c->getEntry(i, j, k, a, b, c)
							    * (sofock->getEntry(i, i)
							        + sofock->getEntry(j, j)
							        + sofock->getEntry(k, k)
							        - sofock->getEntry(a, a)
							        - sofock->getEntry(b, b)
							        - sofock->getEntry(c, c))
							    * (t3c->getEntry(i, j, k, a, b, c)
							        + t3d->getEntry(i, j, k, a, b, c));
						}
					}
				}
			}
